...
| `TBD-AI-002` | Какой benchmark и метрики определяют качество? | Phase 2B live-quality gate | Critical | `BLOCKER_BEFORE_PRODUCTION` | `AI_EVALUATION_SPEC` defines room/window/family/material review; live rights-cleared cases remain unexecuted without credentials | No absolute-quality claim; implementation may close only live-provider-pending | До live Phase 2B/production acceptance | Product / мастер |
...
| TBD-AI-002 | P0 | Какой разрешённый набор фотографий и review определяет приемлемое качество Polza result? | Software criteria defined; live rights-cleared QA still pending and blocks live pass/production claim. | Product / мастер | Открыт |
