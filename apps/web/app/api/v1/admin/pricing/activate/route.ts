...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    await getWebPricing().activateVersion({ actorId: principal.actorId, correlationId: context.correlationId,
      idempotencyKey: `admin:activate:${idempotencyKey}`, priceVersionId: input.priceVersionId, reason: input.reason });
