...
if (-not (Test-Path -LiteralPath $statePath -PathType Leaf) -or
    ...
    throw 'Start the local foundation environment before running catalog pilot acceptance.'
}
...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    Remove-Item Env:CATALOG_PILOT_RUN_ID -ErrorAction SilentlyContinue
}
else {
    $env:CATALOG_PILOT_RUN_ID = $RunId
}

if ([string]::IsNullOrWhiteSpace($RepeatRunId)) {
    Remove-Item Env:CATALOG_PILOT_REPEAT_RUN_ID -ErrorAction SilentlyContinue
}
else {
    $env:CATALOG_PILOT_REPEAT_RUN_ID = $RepeatRunId
}

$scriptPath = Join-Path $PSScriptRoot 'catalog-pilot-acceptance.mjs'
& node.exe $scriptPath
if ($LASTEXITCODE -ne 0) {
    throw "Catalog pilot acceptance failed (exit $LASTEXITCODE)."
}
