...
describe('object storage boundary validation', () => {
  ...
  it('preserves an explicit AMIGO catalog provenance marker', () => {
    const locator = { key: 'catalog/amigo/example.jpg', zone: 'private' } as const;
    const metadata = createProviderMetadata('private', 4, 'b'.repeat(64), 'AMIGO_CATALOG_PILOT');
  ...
  it('preserves the authorized AMIGO preview provenance marker', () => {
    const locator = { key: 'preview/amigo/example.png', zone: 'private' } as const;
    const metadata = createProviderMetadata(
      'private',
      4,
      'd'.repeat(64),
      'AMIGO_AUTHORIZED_PREVIEW',
    );
    ...
    ).toMatchObject({ source: 'AMIGO_AUTHORIZED_PREVIEW', zone: 'private' });
  });
});
