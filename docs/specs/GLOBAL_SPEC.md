...
`OWNER-DECISION-024` authorizes **PHASE 2C — PREMIUM INTERIOR TECH DESIGN, TYPOGRAPHY, MOTION, ERGONOMICS AND RELEASE POLISH** from target-main baseline `bdaa053eee6491a9286355707008a39cbac1abff`. Phase 2B remains `IMPLEMENTATION_COMPLETE_POLZA_LIVE_PROVIDER_PENDING`; its missing live credentials select an honest disabled/pending AI state and do not block presentation work. Phase 2C changes the complete visual and interaction layer, not catalog provenance, pricing authority, private-media controls, provider architecture or production status. Merge and production promotion remain prohibited.
...
- **P2C-DESIGN-008 — MUST:** catalog and material pages preserve only published Supabase facts while providing readable breadcrumbs, search/filter/sort, image proportions, availability/price/manual states and direct calculate/visualize actions without IDs, object paths, source codes or internal enums.
- **P2C-DESIGN-009 — MUST:** the simple calculator presents exactly category, material, width, height and quantity in human Russian labels, keeps millimetres/kopecks and server authority, explains preliminary pricing and renders `MANUAL` as «Стоимость уточнит менеджер» without numeric zero.
- **P2C-DESIGN-010 — MUST:** cart, checkout and request result keep browser-cart/server-recalculation/immutable-request/WhatsApp contracts, show known and manual totals honestly, provide editable item context and never imply payment, accepted order or sent WhatsApp message.
