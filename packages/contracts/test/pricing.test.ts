...
const selection = {
  ...
  productSystemId: '00000000-0000-4000-8000-000000000005',
  quantity: 2,
  widthMm: 800,
};
...
describe('QG-250..QG-257 pricing HTTP contracts', () => {
  ...
  it('keeps quote save input limited to the opaque calculation token', () => {
    expect(quoteSaveRequestSchema.parse({ calculationToken: 'a'.repeat(32) })).toEqual({
      calculationToken: 'a'.repeat(32),
    });
    expect(() =>
      quoteSaveRequestSchema.parse({ calculationToken: 'a'.repeat(32), total: 0 }),
    ).toThrow();
  });
  ...
  it('rejects fake zero totals for PRICE_ON_REQUEST', () => {
    const base = {
      ...
      quantity: 1,
      safeExplanation: 'Цена по запросу.',
      sourceVersion: null,
    ...
    expect(
      pricingCalculationResponseSchema.parse({
        calculationId: selection.catalogVersionId,
        calculationToken: 'b'.repeat(32),
        correlationId: 'contract-test',
        result: base,
      }).result.grandTotalKopecks,
    ).toBeNull();
    expect(() =>
      pricingCalculationResponseSchema.parse({
        calculationId: selection.catalogVersionId,
        calculationToken: 'b'.repeat(32),
        correlationId: 'contract-test',
        result: { ...base, grandTotalKopecks: 0 },
      }),
    ).toThrow();
  });
});
