...
Implementation does not close `TBD-AI-002`, `TBD-PRIV-003`, `TBD-PRIV-005` or `TBD-INFRA-004`. They remain activation gates for the complete family-quality matrix, provider/privacy/legal verification and production deployment evidence. On 2026-08-13 one rights-cleared Zebra case passed the real Supabase/Polza/private-result/delete path; this removes the provider-connectivity unknown but does not invent the remaining roller/horizontal-or-vertical quality, legal or production answers.
...
| `TBD-BIZ-004` | Как устроена фактическая воронка от сообщения до завершённого заказа? | Lead, measure, order, admin, account status | High | `BLOCKER_BEFORE_FEATURE` | Доказанного workflow нет; Phase 1E реализовала только базовую заявку | Заявка использует пять безопасных intake-статусов; `CONFIRMED` не создаёт заказ автоматически | До реализации полноценного Order/production workflow в Phase 1F+ | Владелец бизнеса |
...
| `TBD-ASSORT-002` | Каков полный inventory AMIGO и какие позиции локально активны? | Import, catalog, filters, orderability | High | `RESOLVED` | Phase 1B.2 accepted manifest: 28 categories/56 systems/9 models/1655 variants; `OWNER-DECISION-022` дополнительно исключает из Phase 2A target пять отмеченных категорий и всех потомков как не изготавливаемые бизнесом, не удаляя source records | — | Закрыт 2026-08-04; Phase 2A exact retained/excluded counts фиксирует ETL report | Владелец / менеджер каталога / AMIGO |
...
| `TBD-PRICE-001` | Какая AMIGO-origin PriceVersion активна и проверена? | Preliminary pricing, quote history | Critical | `RESOLVED` | Catalog v2/calculation v5 остаются historical evidence; active public replacement is `amigo-67c782a10449cdb7` under `TBD-PRICE-009`/ADR-0015 | — | Закрыт 2026-08-08 для Phase 1C; replacement activated 2026-08-13 without rewriting history | Владелец / бухгалтер / AMIGO |
...
| `TBD-PRICE-009` | Какая точная AMIGO source version и какие material→model/default mappings покрывают новый public exact-price scope? | Phase 2C public cards/calculator | Critical | `RESOLVED` | Активирована `amigo-67c782a10449cdb7`: 1,428 retained rows проверены, 1,131 имеют current `FROM` + unique calculator mapping, 297 исключены; Zebra 137 | Публикуются только `READY`; generated artifact, ADR-0015, version/cache and rollback verification preserve evidence | Решён 2026-08-13 | Product Owner / OWNER / AMIGO evidence |
...
| `TBD-MIN-PRICE-001` | 1500 рублей — minimum на изделие или заказ? | Pricing minimum | Critical | `RESOLVED_SUPERSEDED` | Исторически minimum применялся к каждой единице по `OWNER-DECISION-003`; `OWNER-DECISION-025` отменил его для всех новых quotes | Старые snapshots воспроизводятся без изменения; новые версии не содержат minimum | Решён 2026-08-02; superseded для новых расчётов 2026-08-12 | Business Owner / Product Owner |
...
| `TBD-DESIGN-001` | Каковы финальные brand/logo/palette/type? | Landing/design/content | High | `SAFE_DEFAULT_AVAILABLE` | `OWNER-DECISION-024` фиксирует light-first palette и Manrope/Cormorant direction; финальные бренд и логотип не утверждены | `PROJECT_NAME` остаётся только внутренним codename; Preview использует нейтральный settings-backed label и admin warning без выдуманного логотипа | До production brand publication Phase 1H | Владелец / дизайнер |
| `TBD-AI-001` | Как сравнивать AI/CV providers/self-hosted? | Phase 2B provider decision | Critical | `RESOLVED` | `OWNER-DECISION-023` and ADR-0014 select Polza Media API/fetch and reject direct Google/SAM/Python/GPU/worker for Phase 2B | Polza remains environment-kill-switched; provider replacement needs a superseding ADR | Решён 2026-08-12 | Architecture / Product |
| `TBD-AI-002` | Какой benchmark и метрики определяют качество? | Phase 2B live-quality gate | Critical | `BLOCKER_BEFORE_PRODUCTION` | `AI_EVALUATION_SPEC` defines room/window/family/material review; one rights-cleared Zebra case passed 2026-08-13, while roller and horizontal-or-vertical cases remain | No absolute-quality claim; the successful technical path does not close the family matrix | До production acceptance | Product / мастер |
| `TBD-AI-003` | Какие file/image limits допустимы? | Upload security/UX/cost | Critical | `RESOLVED` | `OWNER-DECISION-023`/AIVIS-SPEC-033–037 define JPEG/PNG/WebP, 2048 px, 4 MiB normalized and bounded source/decode limits | Reject safely before provider and offer another photo | Решён 2026-08-12 | Engineering / Security / Product |
| `TBD-AI-005` | Как включается generative refinement? | AI UX/consent/cost | High | `RESOLVED` | Geometry/refinement split is superseded; one Polza result starts only after explicit consent/action and both kill switches | Disabled when environment/key/settings disallow | Решён 2026-08-12 | Product / Privacy |
| `TBD-AI-006` | Каковы guest/customer processing limits? | Abuse/cost/queues | Critical | `RESOLVED` | Two successful guest runs/day, one active job/guest, configurable global/concurrency/rate limits and one safe retry are approved | Fail before provider call and retain cart/WhatsApp path | Решён 2026-08-12 | Product / владелец |
...
| `TBD-INFRA-006` | Каков WhatsApp integration mode? | Phase 1E handoff | High | `SAFE_DEFAULT_SELECTED_FOR_PHASE_1E` | Phase 1E fixed-recipient `wa.me`/copy реализован; Business API не подтверждён | Использовать только `79635851036`, safe public link и truthful open/copy events; не заявлять sent/delivery/read | До выбора Business API/automation | Владелец / Architecture |
| `TBD-PRIV-001` | Каков TTL гостевых фото и производных? | Guest upload/delete | Critical | `RESOLVED` | `OWNER-DECISION-023` sets the project input/result default to 24 hours and requires visible notice plus idempotent cleanup | Do not extend media because of cart/request; provider/backup terms remain separate TBD | Решён 2026-08-12 | Privacy / владелец |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-PRICE-001 | P0 | Какая разрешённая AMIGO-origin `PriceVersion` и какие реальные snapshots/локальные правила утверждены для каждой системы и материала? | v2/v5 historical; active public replacement `amigo-67c782a10449cdb7` is governed by ADR-0015. | Владелец / бухгалтер | Решён для Phase 1C 2026-08-08; replacement activated 2026-08-13 |
...
| TBD-PRICE-009 | P0 | Какая dated AMIGO source version и какие material→model/default mappings образуют complete Phase 2C exact-price version? | `amigo-67c782a10449cdb7`; 1,131 complete mappings active, 297 excluded, seven groups/137 Zebra. | Product Owner / OWNER / AMIGO evidence | Решён 2026-08-13 — artifact + ADR-0015 + live Supabase verification |
| TBD-PRICE-011 | P1 | Как показывать клиенту разбивку цены и ручное изменение после замера? | Нужен согласованный уровень прозрачности и объяснение изменений. | Владелец / UX | Открыт |
...
| TBD-MIN-PRICE-001 | P0 | Минимальная стоимость 1500 рублей применяется к одному изделию или ко всему заказу? | Исторически применялась к каждой единице; `OWNER-DECISION-025` отменяет minimum для новых quotes и сохраняет старые snapshots. | Business Owner / Product Owner | Решён 2026-08-02; superseded для новых расчётов 2026-08-12 |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-DESIGN-001 | P0 | Каковы окончательный бренд, логотип, палитра и типографическое направление? | Palette/type закрыты `OWNER-DECISION-024`; бренд/логотип остаются открыты. `PROJECT_NAME` публично не показывается, копирование референсов исключено. | Владелец / дизайнер | Открыт для brand/logo; safe Preview fallback выбран 2026-08-12 |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-AI-001 | P0 | Какие критерии и веса используются для сравнения AI/CV-провайдеров и self-hosted вариантов? | Решён `OWNER-DECISION-023`/ADR-0014: Polza Media API for Phase 2B; direct Google/SAM/Python/GPU/worker rejected. | Architecture / Product | Решён 2026-08-12 |
| TBD-AI-002 | P0 | Какой разрешённый набор фотографий и review определяет приемлемое качество Polza result? | Software criteria defined; one rights-cleared Zebra result passed, but the remaining family matrix and owner/master acceptance still block a full live/production claim. | Product / мастер | Открыт; Zebra evidence 2026-08-13 |
| TBD-AI-003 | P0 | Какие форматы, размеры файла/изображения и лимиты загрузки допустимы? | Решено AIVIS-SPEC-033–037: JPEG/PNG/WebP, metadata strip, 2048 px, ≤4 MiB normalized, bounded source/decode. | Engineering / Security / Product | Решён 2026-08-12 |
...
| TBD-AI-005 | P0 | Generative refinement включается только по явному действию, по умолчанию или недоступен в MVP? | Решено: split superseded; one AI result only after explicit consent/action and environment/database kill switches. | Product / Privacy | Решён 2026-08-12 |
| TBD-AI-006 | P0 | Какие лимиты обработки действуют для гостя и клиента? | Решено: 2 successful/day, 1 active/guest, configured global/concurrency/rate limits, idempotency and bounded retry. | Product / владелец | Решён 2026-08-12 |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-PRIV-001 | P0 | Каков точный срок автоматического хранения гостевых оригиналов и результатов? | Решено `OWNER-DECISION-023`: 24 часа по умолчанию, visible notice, bounded cleanup; Phase 2B masks absent. | Privacy / владелец | Решён 2026-08-12 |
...
До synthetic/local Foundation не было P0 со статусом `BLOCKER_BEFORE_FOUNDATION`. `QG-147/148` закрыты письменным решением Product Owner, а `QG-149`–`158` подтверждают завершение Phase 1A 2026-08-02. Phase 1B.2 закрыла `TBD-ASSORT-002`, `TBD-ASSORT-006` и catalog-version aspect `TBD-PRICE-001`; Phase 1C дополнила его активной расчётной версией. Phase 2C закрыла exact replacement gate `TBD-PRICE-009`; ранее ошибочно дублированный breakdown question получил уникальный `TBD-PRICE-011`. В реестре остаются 98 открытых из 122 исторических ID.
...
Phase 1A–1F завершены; `OWNER-DECISION-019` разрешает только Phase 1F.1 repair и deployment templates без deployment. `OWNER-DECISION-020` не закрывает AI provider/privacy/model-quality gates и не разрешает фото или provider calls. Customer accounts, runtime AI, final design, production deployment и Phase 1G не разрешены автоматически.
