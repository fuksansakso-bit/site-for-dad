...
export const pricingValidationDetailSchema = z.object({
...
const publicPricingProfileSchema = z.object({
...
export const configuratorBootstrapResponseSchema = z.object({
  catalogVersionId: z.uuid(),
...
export const pricingResultSchema = z.object({
  appliedOverrides: z.array(z.object({
    amountMinor: z.number().int().nonnegative(), id: z.uuid(), reason: z.string().min(1).max(512),
  }).strict()).max(8),
...
export const quoteSaveRequestSchema = z.object({ calculationToken: z.string().min(32).max(64) }).strict();
...
export const pricingAdminCommandSchema = z.object({
  priceVersionId: z.uuid(),
  reason: z.string().trim().min(3).max(512),
}).strict();

export const pricingOverrideSetSchema = z.object({
  amountMinor: z.number().int().positive().max(Number.MAX_SAFE_INTEGER),
  materialVariantId: z.uuid(),
  reason: z.string().trim().min(3).max(512),
}).strict();

export const pricingOverrideRemoveSchema = z.object({
  materialVariantId: z.uuid(),
  reason: z.string().trim().min(3).max(512),
}).strict();

export const pricingAdminOverviewResponseSchema = z.object({
  activePriceVersionId: z.uuid().nullable(),
  audit: z.array(z.object({
    action: z.string().min(1).max(128), actorId: z.uuid().nullable(),
    createdAt: z.iso.datetime({ offset: true }), outcome: z.string().min(1).max(32),
    reasonCode: z.string().min(1).max(128), targetId: z.string().min(1).max(255),
  }).strict()).max(30),
  correlationId: z.string().min(8).max(128),
  versions: z.array(z.object({
    activatedAt: z.iso.datetime({ offset: true }).nullable(), changeCount: z.number().int().nonnegative(),
    createdAt: z.iso.datetime({ offset: true }), fixtureCount: z.number().int().nonnegative(),
    id: z.uuid(), maximumDeviationMinor: z.number().int().nonnegative().nullable(),
    parityStatus: z.enum(['FAILED', 'PASSED', 'PENDING']).nullable(), ruleCount: z.number().int().nonnegative(),
    sourceVersion: z.string().max(160).nullable(), status: z.string().min(1).max(32),
    unsupportedCount: z.number().int().nonnegative(), versionNumber: z.number().int().positive(),
  }).strict()).max(12),
}).strict();

export const pricingAdminMutationResponseSchema = z.object({
  correlationId: z.string().min(8).max(128),
