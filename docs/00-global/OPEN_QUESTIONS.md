...
| `TBD-ASSORT-002` | Каков полный inventory AMIGO и какие позиции локально активны? | Import, catalog, filters, orderability | High | `EXTERNAL_AMIGO_DATA_REQUIRED` | Dated Phase 1B.2 discovery доказал текущие 28 categories/56 systems/9 models/1655 variants при 0 failures; вопрос остаётся открыт до принятого PostgreSQL/media Full Catalog Import Manifest, coverage audit и локальной activation/orderability | До activation используется последняя активная pilot version; новые records только staged | До фактического Phase 1B.2 full manifest/acceptance; точная orderability может остаться Phase 1C/manual | Владелец / менеджер каталога / AMIGO |
...
| `TBD-SOURCE-AMIGO-002` | Каков разрешённый полный export/transport AMIGO и процесс обновления? | Phase 1B.2 full import, Phase 1C price sync | Critical | `EXTERNAL_AMIGO_DATA_REQUIRED` | Public-page fallback частично решён: dated discovery прошёл 114 safe paths с semantic source version и 0 failures; official API/export/file/schema всё ещё не доказаны | Последняя активная локальная версия; import остаётся staged и останавливается при credential/CAPTCHA/access/identity/parser blockers | Public-page aspect доказан 2026-08-03; official export aspect остаётся открытым без evidence | Владелец / AMIGO / менеджер каталога |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-ASSORT-002 | P0 | Как выглядит полный проверенный inventory всех текущих AMIGO source categories, systems, models, materials и variants и какие из них локально публикуются/заказываются? | Discovery evidence: 28 categories, 56 systems, 9 models, 1655 variants, 2940 material media links and 1664 price records on 2026-08-03. Вопрос закрывается только accepted full import manifest/coverage/activation evidence; точная orderability не предполагается. | Владелец / менеджер каталога | В работе Phase 1B.2; discovery доказан, import/acceptance не завершены |
...
Phase 1B.1, разрешённая `OWNER-DECISION-010`, завершена на owner-authorized public-page transport со stable IDs и frozen 32-item manifest. Phase 1B.2 dated discovery 2026-08-03 доказал public-page aspect `TBD-SOURCE-AMIGO-002` и текущий source inventory aspect `TBD-ASSORT-002`, но оба ID остаются открытыми до official-export evidence (если оно существует) и accepted full PostgreSQL/media import/coverage/activation соответственно. Phase 1C и любая следующая фаза не разрешены автоматически.
