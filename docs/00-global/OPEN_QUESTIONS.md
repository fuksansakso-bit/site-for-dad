...
| `TBD-SOURCE-AMIGO-002` | Каков разрешённый export/transport AMIGO и процесс обновления? | Phase 1B import, Phase 1C price sync | Critical | `EXTERNAL_AMIGO_DATA_REQUIRED` | PostgreSQL target, authority split и source→diff→owner/admin activation topology определены `OWNER-DECISION-008/009`, но конкретный transport/schema/import evidence не доказаны | Проверенный ручной импорт разрешённого файла; никакого обхода доступа | До первой Phase 1B загрузки AMIGO | Владелец / AMIGO / менеджер каталога |
...
| TBD-SOURCE-AMIGO-002 | P0 | Может ли AMIGO предоставлять разрешённую выгрузку каталога/цен, в каком формате и каков процесс обновления? | PostgreSQL target, field ownership и обязательный staged diff/approval/activation pipeline известны; всё ещё нужны sample/schema, способ доставки, ответственный, ошибки, version marker и evidence конкретного import batch. | Владелец / AMIGO / менеджер каталога | Открыт; authority/serving решены `OWNER-DECISION-008/009`, transport/evidence нет |
