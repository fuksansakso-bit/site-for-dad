...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    await getWebPricing().verifyParity({ actorId: principal.actorId, correlationId: context.correlationId,
      idempotencyKey: `admin:parity:${idempotencyKey}`, priceVersionId: input.priceVersionId, reason: input.reason });
