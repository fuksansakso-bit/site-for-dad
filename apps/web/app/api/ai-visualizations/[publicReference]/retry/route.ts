...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const { data: existingAttempt, error: attemptError } = await client
    ...
    const source = await getOwnedAiJob(client, publicReference, guest.hash);
    let target = source;
    if (source.status === 'SUCCEEDED') {
      target = await cloneSucceededJobForVariant(
        client,
        config,
        source,
        guest.hash,
      ...
      if (target.id === source.id) {
        return NextResponse.json(safeJobPayload(source, true), {
          headers: { 'Cache-Control': 'no-store', 'X-Correlation-ID': requestCorrelationId },
        });
      }
    }
