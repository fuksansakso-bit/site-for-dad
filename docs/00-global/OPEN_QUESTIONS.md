...
| `TBD-PRICE-001` | Какая AMIGO-origin PriceVersion активна и проверена? | Preliminary pricing, quote history | Critical | `RESOLVED` | Active PriceVersion v2 `9fdc0a74-9fab-4d63-b4b6-015f534e117d` содержит 1 664 current-run material/model card/base/price-from records; formula/compatibility/parity остаются отдельными TBD | — | Закрыт 2026-08-04 для catalog PriceVersion; dimensional pricing остаётся gated | Владелец / бухгалтер / AMIGO |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-PRICE-001 | P0 | Какая разрешённая AMIGO-origin `PriceVersion` и какие реальные snapshots/локальные правила утверждены для каждой системы и материала? | Active PriceVersion v2 `9fdc0a74-9fab-4d63-b4b6-015f534e117d` pins 1 664 current-run material/model card/base/price-from revisions and separate local override semantics. Карточная цена не является расчётной матрицей; formula/compatibility/parity остаются `TBD-PRICE-002`–`006`, `TBD-MECHANISM-001`, `TBD-PRICE-SOURCE-001`. | Владелец / бухгалтер | Решён — 2026-08-04, [PRICING_SOURCE_POLICY](PRICING_SOURCE_POLICY.md) 1.9.0 |
...
Phase 1B.1 и отдельно разрешённая `OWNER-DECISION-012` Phase 1B.2 завершены на owner-authorized public-page transport. Accepted full PostgreSQL/media manifest, coverage and activation закрывают `TBD-ASSORT-002`; `TBD-SOURCE-AMIGO-002` остаётся открыт только для official API/export/file/schema aspect, если такой канал существует. Phase 1C и любая следующая фаза не разрешены автоматически.
