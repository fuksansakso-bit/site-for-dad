...
| Продукт | `PROJECT_NAME` как внутренний codename; публичный бренд остаётся `TBD-DESIGN-001` |
...
- [Phase 2C final design plan](../06-plans/active/PHASE_2C_FINAL_DESIGN_PLAN.md)
...
| 0.29.0 | 2026-08-12 | `OWNER-DECISION-024` authorizes Phase 2C from target-main `bdaa053`: an original light-first premium interior-tech system, local Manrope/Cormorant typography, bounded first-visit starfield, complete landing/public/admin responsive redesign, truthful states, accessibility/performance/visual gates and Vercel Preview delivery. Runtime data/provider/price boundaries remain unchanged; merge and production stay prohibited. |
| 0.30.0 | 2026-08-12 | `OWNER-DECISION-025` supersedes the 1,500-ruble minimum and manager-price fallback for new quotes. Every public material now requires a versioned AMIGO `FROM` price and a verified material/model calculation path; the customer selects a material and enters only width/height, while the server performs and snapshots the calculation. Incomplete priceability blocks publication; historical quotes remain immutable. |
...
Источник `OWNER-DECISION-*` — письменные решения Product Owner от 2026-08-02, 2026-08-03, 2026-08-08, 2026-08-09 и 2026-08-12. Они задают бизнес- и архитектурные границы; implementation scope расширяется только явным transition decision. `OWNER-DECISION-021` заменяет активную Phase 1F.1 траекторию только в пределах Phase 2A simplification; Phase 1G, AI и финальный редизайн не разрешены.
...
- **OWNER-DECISION-003 — MUST:** исторически минимальная стоимость 1500 рублей применялась к каждому отдельно изготавливаемому изделию, а не ко всему заказу: изделие с результатом формулы 1100 рублей оценивалось в 1500 рублей, два таких изделия — в 3000 рублей. `OWNER-DECISION-025` отменяет это правило только для новых расчётов и заявок; уже сохранённые snapshots продолжают воспроизводиться по исторической версии без скрытого пересчёта.
...
- **OWNER-DECISION-019 — MUST:** Product Owner authorizes only Phase 1F.1 **MVP FUNCTIONAL COMPLETION, CONFIGURATOR COVERAGE, CART REPAIR, STAFF AUTH AND MOBILE BASELINE** on branch `phase/1f1-mvp-functional-completion` from merged-main commit `289b1baef0b53ac7da457098353ee5e7c1e1953f`. All published `MaterialVariant` records MUST be reachable through bounded server-side configurator search/cursor pagination without a hardcoded/browser-loaded full subset. Exact compatibility is never guessed: automatically priced and compatible manual-price combinations are selectable; missing mapping is staff diagnostics; incompatible combinations are denied. Unknown price becomes `PRICE_ON_REQUEST` or `MANUAL_REVIEW_REQUIRED`. `/admin/configurator-coverage` MUST expose filtered counts/reasons and validated, versioned, audited local mapping/manual/hide/bulk commands that sync cannot silently overwrite. `/configure` and `/preview` MUST add only a server-created immutable `QuoteSnapshot` to the guest cart with idempotency and honest eligibility. Public UI MUST replace technical statuses/IDs/errors with Russian actionable labels and satisfy the critical 320–430 px route baseline. Production staff authentication MUST be per-person login/e-mail plus password using Argon2id, rotating hash-only sessions, HttpOnly/SameSite/production-Secure cookie, CSRF/origin/rate/lock/audit/logout/revoke/forced-first-change controls and last-OWNER/role guards; customer accounts remain absent and Phase 1F OTP/development keys are local/test only. Ubuntu 24.04 Docker Compose/Nginx/backup/rollback templates MAY be prepared but production deployment, credentials and public PII launch MUST NOT occur. Client-photo upload, Polza calls, AI generation, SAM, Python AI service, payment, final design, Motion/Three.js/starfield and Phase 1G runtime MUST NOT begin. Completion grants no next-phase authority.
...
- **OWNER-DECISION-024 — MUST:** Product Owner authorizes exactly **PHASE 2C — PREMIUM INTERIOR TECH DESIGN, TYPOGRAPHY, MOTION, ERGONOMICS AND RELEASE POLISH** on branch `phase/2c-final-premium-design` from target `second-github/main` commit `bdaa053eee6491a9286355707008a39cbac1abff`, protected locally by tag `pre-final-design-phase`. The phase MUST deliver one original light-first system across every required public and staff route using centrally governed palette/type/spacing/radius/shadow/motion tokens, local `next/font` Manrope and Cormorant Garamond with Cyrillic, an optional bounded first-visit starfield, a complete source-backed landing, final catalog/material/calculator/cart/checkout/request/AI/admin UX, responsive navigation, honest states, WCAG 2.2 AA checks, measured performance and route/browser/visual verification. Existing Supabase catalog/settings/portfolio/staff contracts, server price calculation, immutable request/cart evidence, Polza adapter, private photo ownership/retention, rights/provenance and feature switches MUST remain authoritative. Final brand/logo remain open: `PROJECT_NAME` MUST remain an internal codename and MUST NOT appear as a public brand; Preview uses a neutral settings-backed fallback and the admin warns that brand setup is required before production. No fake metrics, reviews, ratings, address, hours, portfolio work, scarcity or unavailable business claims MAY be invented. Phase 2C MUST NOT add a new provider or dependency-heavy visual runtime, direct Google API/SDK/key, SAM/masks/segmentation, Python/GPU/worker service, customer accounts, payment, new AMIGO discovery/import, production data deletion, merge or production promotion. Delivery is limited to local evidence, a target-account Vercel Preview and an unmerged Draft PR in `fuksansakso-bit/site-for-dad`.
- **OWNER-DECISION-025 — MUST:** Product Owner cancels the local 1,500-ruble minimum and the public «стоимость уточнит менеджер» fallback for every new calculation created after activation of the replacement `PriceVersion`. AMIGO remains the sole base-price authority: every public material card MUST show its exact versioned source-card `FROM` amount as «от … ₽», and a material MUST NOT remain published unless it has both an `AVAILABLE` AMIGO price fact and an unambiguous source-material/source-model mapping that can produce a server-validated dimensional quote. After material selection the customer enters only integer width and height; quantity defaults to one and MAY be adjusted later in the cart through a new server recalculation. The browser MUST NOT call AMIGO or trust client-supplied model/options/price; a server-only adapter MAY use only the observed allowlisted public calculator HTTPS origin and paths, with strict timeout, rate/concurrency bounds, response schema validation, source-version evidence and an exact local cache, without login, CAPTCHA bypass, hidden credentials or supplier frontend code. A calculation MUST store material/model/source version, dimensions, exact source response amount and verification time; deviation from the same AMIGO context MUST remain at most one ruble. Missing/stale/ambiguous card price or calculation mapping MUST fail publication or calculation explicitly and MUST NOT be replaced by zero, an invented area formula, a local override, `MANUAL`, `PRICE_ON_REQUEST` or manager-copy. Existing immutable quote/order snapshots retain their historical rules and amounts. This decision supersedes conflicting new-quote portions of `OWNER-DECISION-003`, `OWNER-DECISION-009`, `OWNER-DECISION-013`, `OWNER-DECISION-021` and `OWNER-DECISION-024` only for this narrow Phase 2C catalog/pricing repair; it does not authorize uncontrolled discovery/import, payment, merge or production promotion.
...
### 6.1.1. Phase 2C final experience contract

- **P2C-DESIGN-001 — MUST:** every public and staff surface uses one original `PREMIUM INTERIOR TECH` language that is light-first, calm, tactile and architectural rather than a generic SaaS dashboard, copied reference, dark-first theme or ornamental showroom.
- **P2C-DESIGN-002 — MUST:** central tokens define the approved warm paper/white surfaces, near-black text, muted text, walnut/bronze/sage accents, success/warning/error colors, alpha borders, typography, spacing, radii, shadows, layout widths, focus and motion; route CSS MUST consume semantic tokens instead of independent visual systems.
- **P2C-DESIGN-003 — MUST:** `next/font` self-hosts Manrope for UI/body and Cormorant Garamond for display text with Cyrillic subsets and safe fallbacks; no runtime font CDN or unreadable decorative typography is allowed.
- **P2C-DESIGN-004 — MUST:** public brand text comes from published `SiteSettings`; until final brand/logo are approved, a neutral Preview-only fallback is used, `PROJECT_NAME` is not rendered publicly and `/admin/settings` shows «Укажите название бренда перед production-запуском».
- **P2C-DESIGN-005 — MUST:** the starfield is an optional 1.8–2.6-second first-visit overlay with visible skip, safe `intro_seen_v1`, parallel content loading, pause/cleanup on hidden/unmount, static weak-device fallback and immediate reduced-motion bypass; it MUST NOT delay semantic content beyond three seconds or require WebGL.
- **P2C-DESIGN-006 — MUST:** `/` contains a complete header, truthful hero and CTAs, real categories, three-step process, real published popular materials, calculator teaser, conditionally enabled AI section, separately approved business portfolio, confirmed service/trust/partner/FAQ content, final CTA and footer; unknown or unavailable data is omitted rather than invented.
- **P2C-DESIGN-007 — MUST:** public desktop navigation and the mobile bottom navigation (`Главная`, `Каталог`, `Рассчитать`, `Корзина`) expose the active guest journey with correct current state, safe-area support, at least 44×44 CSS-pixel targets and no content obstruction.
- **P2C-DESIGN-008 — MUST:** catalog and material pages preserve only active published Supabase facts while providing readable breadcrumbs, search/filter/sort, image proportions, availability and current AMIGO `FROM` price; incomplete mapping/manual/zero rows are absent and direct calculate/visualize actions expose no IDs, object paths, source codes or internal enums.
- **P2C-DESIGN-009 — MUST:** under `OWNER-DECISION-025`, the initial simple calculator presents premium category/material selection followed by exactly width and height in human Russian labels; quantity starts at one, millimetres/kopecks and active-version server authority are preserved, and success is a positive exact AMIGO amount without local minimum, `MANUAL`, manager-price or numeric-zero state.
- **P2C-DESIGN-010 — MUST:** cart, checkout and request result keep browser-cart/server-recalculation/immutable-request/WhatsApp contracts, show active exact totals for new items, preserve any historical snapshot honestly, provide editable item context and never imply payment, accepted order or sent WhatsApp message.
- **P2C-DESIGN-011 — MUST:** AI presentation supports select/upload/consent/processing/result plus safe disabled, expired, deleted, limit, provider, offline and retry states; entry CTAs are hidden when the feature flag is off and the page explains availability without exposing provider secrets or technical internals.
- **P2C-DESIGN-012 — MUST:** `/admin/login` and every required `/admin` route use one restrained Russian business shell with role-safe navigation/actions, dense but readable tables/cards/forms and no starfield or marketing decoration.
- **P2C-DESIGN-013 — MUST:** a centralized Russian presentation mapping converts every user-visible category, price mode, availability, role, request and AI status; raw enum values, UUIDs, hashes, storage names and English implementation labels MUST NOT leak to normal public/admin UI.
- **P2C-DESIGN-014 — MUST:** every data-dependent surface defines proportionate loading skeleton, empty, validation, unavailable/offline, permission, failure, success and disabled states with recovery action where recovery is possible and no false successful state.
- **P2C-DESIGN-015 — MUST:** WCAG 2.2 AA is the release target: semantic landmarks/headings/forms, keyboard completion, visible focus, sufficient contrast, programmatic names/descriptions/errors/status, no color-only meaning, 200% zoom/reflow, reduced motion and usable touch targets are verified.
- **P2C-DESIGN-016 — MUST:** route layouts are explicitly checked at 320×568, 360×800, 375×812, 390×844, 430×932, 768×1024, 1024×768, 1280×800, 1440×900 and 1920×1080 without horizontal overflow, clipped controls or unreachable actions.
- **P2C-DESIGN-017 — MUST:** approved responsive images use `next/image` or an equally bounded native fallback with explicit dimensions/sizes; initial JavaScript, client-component boundaries, intro chunk, image transfer, LCP/CLS/INP proxies and production build output are measured and documented.
- **P2C-DESIGN-018 — MUST:** SEO metadata describes only the confirmed local blinds business, service region and available routes; rating, review count, address, opening hours, prices, guarantees or structured-data facts MUST NOT be fabricated.
- **P2C-DESIGN-019 — MUST:** analytics, if emitted, is limited to privacy-safe action names and coarse states for navigation, calculation, cart, request, WhatsApp open/copy and AI lifecycle; names, phones, free text, photo/object URLs, references, material source locators and secrets MUST NOT be sent.
- **P2C-DESIGN-020 — MUST:** acceptance covers every required route, scenarios A–G, the fixed viewport matrix, thirteen visual baselines, unit/component/browser/accessibility/performance/build/security checks, a directly smoke-tested target-account Vercel Preview and an unmerged Draft PR; credential-dependent evidence remains labelled and production stays blocked.
...
- **FR-CALC-004 — MUST:** денежные значения хранятся целыми копейками без `float`; например, 3615 рублей представляются как 361500 копеек.
...
- **FR-CALC-009 — MUST:** для исторических calculation snapshots минимальная стоимость 1500 рублей воспроизводится отдельно для каждой изготавливаемой единицы по `OWNER-DECISION-003`. Новые расчёты после активации версии по `OWNER-DECISION-025` MUST NOT применять этот минимум.
...
- **FR-CALC-020 — MUST:** расчёт использует только активную подтверждённую `PriceVersion`. Исторические/manual scopes сохраняют прежний честный статус, но public scope, активированный по `OWNER-DECISION-025`, MUST fail closed без суммы, если точный source-backed расчёт недоступен, и MUST NOT показывать manager-price placeholder.
...
- **FR-CALC-022 — MUST NOT:** браузерный запрос цены не обращается к AMIGO и не передаёт внешний model/options/price. До `OWNER-DECISION-025` public runtime не зависел от AMIGO; новый exact-price scope MAY обращаться только через bounded server-only adapter и exact local cache по правилам `FR-CALC-028`.
...
- **FR-CALC-025 — MUST NOT:** новая `PriceVersion`, активированная по `OWNER-DECISION-025`, применяет локальный minimum 1500 рублей или любой другой недоказанный минимум; расчёт возвращает точную source-backed сумму в целых копейках.
- **FR-CALC-026 — MUST:** каждая публичная карточка материала показывает «от {AMIGO source-card amount} ₽» из точной активной source revision, включая `sourceVersion`, `capturedAt`, source material identity и административную активацию; derived dimensional result MUST NOT подменять card `FROM` amount.
- **FR-CALC-027 — MUST:** после выбора опубликованного материала клиент вводит только `widthMm` и `heightMm`; quantity для первоначального расчёта равна одному. Server independently resolves material, category, AMIGO source material/model, mandatory default options and active price version before calculation.
- **FR-CALC-028 — MUST:** exact-price adapter работает только server-side через allowlisted HTTPS origin/path, strict connect/overall timeout, bounded rate/concurrency, schema/amount validation and exact cache keyed by source version + material + model + dimensions + normalized options. Ни browser, ни URL/query/body клиента не выбирают внешний host, model, options или amount.
- **FR-CALC-029 — MUST:** материал без `AVAILABLE` AMIGO `FROM` amount, approved local media, однозначного calculator mapping или успешной activation validation MUST NOT входить в `public_materials`/public category counts. UI MUST NOT заменять отсутствующую цену нулём, придуманной area-формулой, `MANUAL`, `PRICE_ON_REQUEST` или текстом про менеджера.
- **FR-CALC-030 — MUST:** каждый новый calculation snapshot сохраняет source version, source material/model identity, width/height, normalized server defaults, exact source result, cache/source outcome and verified time. Сравнение с тем же AMIGO context допускает не более 100 копеек расхождения.
- **FR-CALC-031 — MUST:** существующие quote/order snapshots остаются byte-for-byte неизменными и продолжают воспроизводиться с историческим минимумом/правилами; добровольный пересчёт создаёт новый snapshot по текущей версии, а не изменяет старый.
