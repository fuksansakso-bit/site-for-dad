import { pricingAdminCommandSchema, pricingAdminMutationResponseSchema } from '@project-name/contracts';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const [principal, input] = await Promise.all([requireCatalogAdminPrincipal(), parsePricingJson(request, pricingAdminCommandSchema)]);
    await getWebPricing().rejectVersion({ actorId: principal.actorId, correlationId: context.correlationId,
      idempotencyKey: `admin:reject:${idempotencyKey}`, priceVersionId: input.priceVersionId, reason: input.reason });
