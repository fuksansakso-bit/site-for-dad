...
| `TBD-ASSORT-002` | Каков полный inventory AMIGO и какие позиции локально активны? | Import, catalog, filters, orderability | High | `EXTERNAL_AMIGO_DATA_REQUIRED` | Phase 1B.1 подтвердил frozen 32-ID manifest; `OWNER-DECISION-012` разрешил Phase 1B.2 discovery/import, но полный inventory/categories/compatibility/orderability считается доказанным только после принятого manifest и coverage audit | До activation используется последняя активная pilot version; новые records только staged | До фактического Phase 1B.2 full manifest/acceptance; точная orderability может остаться Phase 1C/manual | Владелец / менеджер каталога / AMIGO |
...
| `TBD-SOURCE-AMIGO-002` | Каков разрешённый полный export/transport AMIGO и процесс обновления? | Phase 1B.2 full import, Phase 1C price sync | Critical | `EXTERNAL_AMIGO_DATA_REQUIRED` | `OWNER-DECISION-012` разрешает расширить proven public-page adapter на доступный полный каталог при transport/stop/manifest gates; official API/export/file/schema всё ещё не доказаны | Последняя активная локальная версия; discovery остаётся staged и останавливается при credential/CAPTCHA/access/identity/parser blockers | До фактического dated Phase 1B.2 transport discovery; official export aspect остаётся открытым без evidence | Владелец / AMIGO / менеджер каталога |
...
| `TBD-PRICE-001` | Какая AMIGO-origin PriceVersion активна и проверена? | Preliminary pricing, quote history | Critical | `EXTERNAL_AMIGO_DATA_REQUIRED` | Pilot PriceVersion v1 активна только для 32 source card prices «от»; formula/rule matrix, parity cases и полный verified price set для расчёта отсутствуют | `PRICE_ON_REQUEST` / ручной расчёт; никогда `0` | До numeric pricing activation в Phase 1C | Владелец / бухгалтер / AMIGO |
...
| `TBD-PRICE-007` | Кто и как вводит PriceVersion в действие? | Admin pricing activation, audit | Critical | `RESOLVED` | Только `OWNER`/`ADMIN` после diff и подтверждения; все активации audited, `OWNER-DECISION-002` | — | Закрыт 2026-08-02; реализация остаётся Phase 1C | Product Owner / Business Owner |
...
| `TBD-MIN-PRICE-001` | 1500 рублей — minimum на изделие или заказ? | Pricing minimum | Critical | `RESOLVED` | Minimum применяется к каждой отдельно изготавливаемой единице изделия; `OWNER-DECISION-003` | Не реализовывать в Phase 1A; до остальных pricing gates использовать ручной расчёт | Закрыт 2026-08-02; activation остаётся Phase 1C | Business Owner / бухгалтер |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-ASSORT-002 | P0 | Как выглядит полный проверенный inventory всех текущих AMIGO source categories, systems, models, materials и variants и какие из них локально публикуются/заказываются? | Phase 1B.1 доказал exact 32-ID manifest, 32 published variants and 59 mapped assets in active CatalogVersion v1. `OWNER-DECISION-012` разрешает получить remaining source IDs/categories/properties/compatibility/prices/assets в Phase 1B.2; вопрос закрывается только accepted full manifest и coverage evidence, а точная orderability не предполагается. | Владелец / менеджер каталога | В работе Phase 1B.2; не решён до full acceptance evidence |
...
| TBD-SOURCE-AMIGO-002 | P0 | Может ли AMIGO предоставлять разрешённую полную выгрузку каталога/цен, в каком формате и каков процесс обновления? | PostgreSQL target и staged pipeline известны. `OWNER-DECISION-012` разрешает Phase 1B.2 full public-page fallback через existing adapter при dated path/identity/parser/coverage evidence; official export/file sample/schema, если они существуют, всё ещё нельзя предполагать. | Владелец / AMIGO / менеджер каталога | В работе Phase 1B.2 для public-page transport; official export aspect открыт без evidence |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-PRICE-001 | P0 | Какая разрешённая AMIGO-origin `PriceVersion` и какие реальные snapshots/локальные правила утверждены для каждой системы и материала? | Active pilot PriceVersion v1 pins 32 source card prices «от» and separate local override semantics. Для закрытия numeric pricing нужны полный verified set, formula/rule versions, compatibility inputs, control examples and parity evidence; карточная цена не является расчётной матрицей. | Владелец / бухгалтер | Открыт для Phase 1C numeric pricing; pilot price publication завершена 2026-08-03 |
...
| TBD-MIN-PRICE-001 | P0 | Минимальная стоимость 1500 рублей применяется к одному изделию или ко всему заказу? | Решено: к каждой отдельно изготавливаемой единице; 1100 → 1500, две единицы → 3000. | Business Owner / бухгалтер | Решён — 2026-08-02, `OWNER-DECISION-003`; не реализуется в Phase 1A |
...
Phase 1B.1, разрешённая `OWNER-DECISION-010`, завершена на owner-authorized public-page transport со stable IDs и frozen 32-item manifest. `OWNER-DECISION-012` разрешает проработать `TBD-SOURCE-AMIGO-002` и `TBD-ASSORT-002` только в Phase 1B.2; сами TBD не считаются решёнными до dated transport/full manifest/acceptance evidence. Phase 1C и любая следующая фаза не разрешены автоматически.
