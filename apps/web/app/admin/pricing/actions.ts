...
function text(form: FormData, name: string, maximum = 512): string {
  ...
  if (typeof value !== 'string' || value.trim().length < 3 || value.length > maximum)
    throw new Error('PRICING_ADMIN_INPUT_INVALID');
  return value.trim();
}
...
function minor(form: FormData): number {
  ...
  if (typeof value !== 'string' || !/^\d{1,9}(?:[.,]\d{1,2})?$/u.test(value))
    throw new Error('PRICING_ADMIN_INPUT_INVALID');
...
function identity(action: string) {
  const id = randomUUID();
  return {
    correlationId: `pricing-admin-${action}-${id}`,
    idempotencyKey: `pricing:admin:${action}:${id}`,
  };
}
...
export async function verifyPricingParity(form: FormData): Promise<never> {
  return finish(async () => {
    ...
    await getWebPricing().verifyParity({
      actorId: principal.actorId,
      ...identity('parity'),
      priceVersionId: uuid(form, 'priceVersionId'),
...
export async function rejectPriceVersion(form: FormData): Promise<never> {
  return finish(async () => {
    const principal = await requireCatalogAdminPrincipal();
    await getWebPricing().rejectVersion({
      actorId: principal.actorId,
      ...identity('reject'),
      priceVersionId: uuid(form, 'priceVersionId'),
      reason: text(form, 'reason'),
...
export async function setPricingOverride(form: FormData): Promise<never> {
  return finish(async () => {
    const principal = await requireCatalogAdminPrincipal();
    await getWebPricing().setLocalOverride({
      actorId: principal.actorId,
      amountMinor: minor(form),
      ...identity('override-set'),
      materialVariantId: uuid(form, 'materialVariantId'),
      reason: text(form, 'reason'),
    });
