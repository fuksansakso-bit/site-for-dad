...
| `TBD-ASSORT-002` | Каков полный inventory AMIGO и какие позиции локально активны? | Import, catalog, filters, orderability | High | `EXTERNAL_AMIGO_DATA_REQUIRED` | `OWNER-DECISION-008/009` определяют field authority и active PostgreSQL public-serving topology, но фактический import manifest/verified набор отсутствует | Phase 1B pilot только на 20–50 проверенных материалах; остальное hidden/review | До Phase 1B pilot data activation; полный набор не блокирует MVP | Владелец / менеджер каталога / AMIGO |
...
| `TBD-PRICE-001` | Какая AMIGO-origin PriceVersion активна и проверена? | Preliminary pricing, quote history | Critical | `EXTERNAL_AMIGO_DATA_REQUIRED` | AMIGO base-price authority и PostgreSQL-only runtime path определены `OWNER-DECISION-008/009`, но активных verified `CatalogVersion`/`PriceVersion`/rules нет | `PRICE_ON_REQUEST` / ручной расчёт; никогда `0` | До numeric pricing activation в Phase 1C | Владелец / бухгалтер / AMIGO |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-ASSORT-002 | P0 | Как выглядит полный проверенный inventory всех текущих AMIGO source categories, systems, models, materials и variants и какие из них локально публикуются/заказываются? | Authority split и PostgreSQL public-serving pipeline определены `OWNER-DECISION-008/009`; для закрытия всё ещё нужны import manifest, source IDs, артикулы, свойства, совместимости, pricing/orderability, asset mapping и одобренная активная `CatalogVersion`; импорт не равен публикации. | Владелец / менеджер каталога | Открыт; dynamic scope, authority и serving topology подтверждены 2026-08-02 |
...
| ID | P | Вопрос | Зачем нужен ответ / критерий закрытия | Владелец ответа | Статус |
|---|---|---|---|---|---|
| TBD-PRICE-001 | P0 | Какая разрешённая AMIGO-origin `PriceVersion` и какие реальные snapshots/локальные правила утверждены для каждой системы и материала? | AMIGO является authority base price, Business Owner — local override, а runtime использует только совместимые active PostgreSQL `CatalogVersion`/`PriceVersion`; для закрытия нужны полный проверенный набор, source versions, контрольные примеры и owner/admin activation. Публичные «от» цены не являются матрицей. | Владелец / бухгалтер | Открыт; authority/serving определены `OWNER-DECISION-008/009`, active versions отсутствуют |
