...
| Поле | Значение |
|---|---|
| Статус | Phase 1D `AUTHORIZED_IN_PROGRESS`; implementation evidence is required before acceptance |
| Версия | 0.2.0 |
| Дата | 2026-08-08 |
...
In scope for Phase 1D: two original local scene profiles, family-specific product geometry, layered SVG rendering, exact asset mapping or labelled evidence fallback, interactive position/view variants, mutable ownership-scoped `StandardPreviewState`, accessible summary and deterministic visual baselines.
...
Phase 1D publishes two owner-created vector scenes: `WINDOW_CLOSEUP` and `ROOM_WINDOW`. Each profile contains stable viewBox/window anchors, protected frame/handle layers and supported family list. The registry is data-driven so later living room, bedroom, kitchen, office, light/dark and day/evening variants can be added without changing renderer selection.
...
Dependencies: catalog/configurator/media/storage/data/API/UX/performance/testing. Phase 1D implementation and visual evidence close the initial scene/profile/family aspect of `TBD-PREVIEW-001`; unimplemented export and broader content profiles remain outside this phase. Rights/attribution and retention governance remain under `TBD-ASSET-AMIGO-003` and `TBD-ASSET-RETENTION-001`. Risks: wrong material, misleading scale/color, generic shape for unsupported family, protected-layer overwrite, stale revoked asset and heavy initial load.
