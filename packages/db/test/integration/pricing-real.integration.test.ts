...
import { createPricingAdapter, PricingStoreError, type PricingAdapter } from '../../src/pricing.js';
...
describe.skipIf(!enabled)('QG-260 Phase 1C real PostgreSQL integration', () => {
  ...
  it('calculates from PostgreSQL, deduplicates, snapshots, overrides, audits and rolls back', async () => {
    ...
    const selection = {
      additionalOptionIds: [], catalogVersionId: bootstrap.catalogVersionId,
      configuratorModelId: profile.configuratorModelId, controlTypeId: profile.optionData.controlTypes[0]?.id ?? '',
      hardwareOptionId: profile.optionData.hardwareOptions[0]?.id ?? '', heightMm: profile.minimumHeightMm,
      materialVariantId: profile.materialVariantId, mountingTypeId: profile.optionData.mountingTypes[0]?.id ?? '',
      productFamilyId: profile.productFamilyId, productSystemId: profile.productSystemId, quantity: 2,
      widthMm: profile.minimumWidthMm,
    };
    const suffix = randomUUID();
    const command = { correlationId: `phase1c-integration-${suffix}`, idempotencyKey: `phase1c:calc:${suffix}`, selection };
    ...
    const quote = await adapter.saveQuote({ calculationToken: first.calculationToken,
      correlationId: command.correlationId, idempotencyKey: `phase1c:quote:${suffix}` });
    expect(await adapter.saveQuote({ calculationToken: first.calculationToken,
      correlationId: command.correlationId, idempotencyKey: `phase1c:quote:${suffix}` })).toEqual(quote);

    const overrideKey = `phase1c:override:set:${suffix}`;
    const override = { actorId: ownerId, amountMinor: 149_000, correlationId: command.correlationId,
      idempotencyKey: overrideKey, materialVariantId: profile.materialVariantId, reason: 'Phase 1C integration minimum boundary.' };
    const overrideId = await adapter.setLocalOverride(override);
    expect(await adapter.setLocalOverride(override)).toBe(overrideId);
    try {
      const changed = await adapter.calculate({ ...command, idempotencyKey: `phase1c:calc:override:${suffix}` });
      expect(changed.result).toMatchObject({ minimumPriceApplied: true, unitBasePriceKopecks: 149_000,
        unitFinalPriceKopecks: 150_000, grandTotalKopecks: 300_000 });
      ...
      await expect(pool.query(`UPDATE quote_snapshot SET correlation_id = 'mutated' WHERE public_token = $1`, [quote.quoteToken])).rejects.toThrow();
    } finally {
      const remove = { actorId: ownerId, correlationId: command.correlationId,
        idempotencyKey: `phase1c:override:remove:${suffix}`, materialVariantId: profile.materialVariantId,
    ...
    await expect(adapter.setLocalOverride({ actorId: ownerId, amountMinor: 150_000,
      correlationId: command.correlationId, idempotencyKey: rollbackKey,
      materialVariantId: '00000000-0000-4000-8000-000000000000', reason: 'Rollback test.' })).rejects.toMatchObject({ code: 'PRICING_NOT_FOUND' } satisfies Partial<PricingStoreError>);
