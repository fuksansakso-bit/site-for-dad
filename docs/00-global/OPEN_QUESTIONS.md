...
| `TBD-AI-001` | Как сравнивать AI/CV providers/self-hosted? | Phase 1G provider decision | Critical | `BLOCKER_BEFORE_FEATURE` | `OWNER-DECISION-020` задаёт первый документированный кандидат Polza/Gemini и исключает SAM/Python/GPU из MVP, но contract/quality/privacy evaluation не выполнена | Не вызывать AI и не принимать фото; deterministic preview only | До provider ADR и external processing | Architecture / Product |
