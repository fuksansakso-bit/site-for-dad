...
| `TBD-ASSORT-002` | Каков полный inventory AMIGO и какие позиции локально активны? | Import, catalog, filters, orderability | High | `EXTERNAL_AMIGO_DATA_REQUIRED` | Phase 1B.1 подтвердил и активировал только frozen 32-ID manifest; полный inventory/categories/compatibility/orderability по-прежнему не доказан | Активная 32-item pilot version; всё вне allowlist hidden/unimported | До отдельного Phase 1B.2/full-catalog authorization; pilot больше не блокирует | Владелец / менеджер каталога / AMIGO |
...
| `TBD-SOURCE-AMIGO-002` | Каков разрешённый полный export/transport AMIGO и процесс обновления? | Phase 1B.2 full import, Phase 1C price sync | Critical | `EXTERNAL_AMIGO_DATA_REQUIRED` | `OWNER-DECISION-010` и dated discovery подтверждают bounded public-page transport/stable IDs только для 32-item Phase 1B.1 pilot; official API/export/file/schema всё ещё не доказаны | Последняя активная локальная версия; полный импорт не начинается | До Phase 1B.2/full-catalog expansion | Владелец / AMIGO / менеджер каталога |
...
| TBD-SOURCE-AMIGO-002 | P0 | Может ли AMIGO предоставлять разрешённую полную выгрузку каталога/цен, в каком формате и каков процесс обновления? | PostgreSQL target и staged pipeline известны. Для Phase 1B.1 подтверждён owner-authorized public-page fallback со stable IDs и frozen 32-item manifest; для full catalog всё ещё нужны official export/file sample/schema, delivery owner, errors и source version marker. | Владелец / AMIGO / менеджер каталога | Открыт для Phase 1B.2/full import; Phase 1B.1 transport доказан 2026-08-02 |
...
Phase 1B.1, разрешённая `OWNER-DECISION-010`, завершена на owner-authorized public-page transport со stable IDs и frozen 32-item manifest. `TBD-SOURCE-AMIGO-002` и `TBD-ASSORT-002` остаются открытыми только для полного каталога/Phase 1B.2; никакая следующая фаза не разрешена автоматически.
